...
public class CheckIdCard {
	...
	private Date cacheBirthDate = null; // 出生日期
	...
	public static CheckIdCard getInstance(String number) {
		if (checkIdCard == null) {
			checkIdCard = new CheckIdCard();
		}
		if (null != number) {
			cardNumber = number.trim();
			if (OLD_CARD_NUMBER_LENGTH == number.length()) {
				cardNumber = contertToNewCardNumber(number);
			}
		}
		return checkIdCard;
	}
